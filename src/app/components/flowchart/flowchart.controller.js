(function () {
  ...
  function FlowchartController($element, $window, modelService) {
    ...
    vm.currentlyDraggingNode = false;
    ...
    vm.translateCoordinates = function (x, y, evt) {
      ...
      //TODO figure out if this is still necessary if $element is not the svg
    ...
    vm.nodeMouseDown = function (evt, node) {
      evt.stopPropagation();
      evt.preventDefault();
      vm.currentlyDraggingNode = true;
    };

    vm.nodeMouseMove = function (evt, node) {
      evt.stopPropagation();
      var nodeElement = evt.path[1];

      evt.preventDefault();
      if (vm.currentlyDraggingNode) {
        ...
        vm.model.nodes[node.id][x] = newX;
        vm.model.nodes[node.id][y] = newY;

      }
    };

    vm.nodeMouseUp = function (evt, node) {
      evt.preventDefault();
      evt.stopPropagation();
      vm.currentlyDraggingNode = false;
      console.log('TODO nodeMoveUp now set new position in model');
    };

    vm.resizeEditor = function(){
      var stepSize = 25;
      vm.editorHeight = vm.editorHeight + stepSize;
    };
